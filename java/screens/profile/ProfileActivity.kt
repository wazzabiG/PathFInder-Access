...
class ProfileActivity : BaseActivity() {
    ...
    // ── onResume: refresh all displayed data immediately after returning from
    //   EditProfileActivity (fixes the "update is delayed" issue).
    ...
    private fun refreshProfileUI() {
        ...
        // Reload photo — reset tint first in case we're coming back from a
        // fresh session where no photo exists yet.
        avatarImage.setColorFilter(
            androidx.core.content.ContextCompat.getColor(this, R.color.colorPrimary)
        )
        loadSavedPhoto()
    ...
    /**
     * Remove the photo preference entry for the current user so it won't
     * bleed over to a different user's session after logout.
     */
